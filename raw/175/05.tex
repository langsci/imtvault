...
\label{chapter:dua} 
\section{Die Präferenz für \textit{doch auch} --- Annahmen aus der Literatur und Korpusfrequenzen}\sectionmark{Die Präferenz für \textit{doch auch}}
\label{sec:präferenz}
...
Da jeder Beleg im Kontext angeschaut werden muss, habe ich mir für die Daten in DeReKo eine Möglichkeit der Hochrechnung der Ergebnisse einer Stichprobe zu Nutze gemacht und \textquote{nur} ein Teilkorpus von DECOW betrachtet. Obwohl die Entscheidung für jeden Beleg einzeln vorgenommen wurde, gilt dennoch die Einschränkung, dass es sich sicherlich nicht um absolute Urteile handelt. Das Kriterium, das ich angesetzt habe, ist, ob \textit{doch} und \textit{auch} als MP interpretiert werden \emph{können}. Ich teste, ob die Partikeln jeweils alleine als MPn gebraucht werden \emph{könnten}, nicht ob sie es \emph{müssen} (wenn dies überhaupt zu entscheiden ist). Bei der Abfolge \textit{doch auch} gilt es, auszuschließen, dass \textit{auch} als Adverb \is{Adverb} engen Skopus nimmt; bei der Abfolge \textit{auch doch}, dass \textit{doch} betont auftritt. Ein völliger Ausschluss des Vorkommens einer der \textquote{Dubletten} ist in manchen Äußerungen allerdings nahezu unmöglich. \tabref{tab:836} zeigt die Ergebnisse.
...
Verfolgt man die Absicht, eine Erklärung für die Präferenz von \textit{doch auch} gegenüber \textit{auch doch} zu finden, ist es entscheidend, zu wissen, welche Satzmodi \is{Satzmodus} bzw. Äuße\-rungstypen \is{Illokutionstyp} erfasst werden können müssen. Wie bereits in Abschnitt~\ref{sec:distributionjd} und Abschnitt~\ref{sec:bedhe} angeführt, können sich die gleichen MPn nicht in allen \mbox{(Satz-)}Kon\-tex\-ten kombinieren. Die etablierte Annahme, auf die ich mich hier verlasse, ist die Schnittmengenbedingung \is{Schnittmengenbedingung} aus \citet{Thurmair1989, Thurmair1991}: MPn können sich nur in den Satzmodi kombinieren, in denen sie auch in Isolation auftreten können.\\
...
\end{exe}
Im Rahmen der Untersuchung dieser beiden MPn sind \is{V1-Deklarativsatz} der V1- und \textit{Wo}-VL-De\-kla\-ra\-tiv\-satz \is{Wo-VL-Deklarativsatz} von besonderem Interesse (vgl.\ auch meine gesonderte Betrachtung in Abschnitt~\ref{sec:Rand}).
...
\end{exe}
In Exklamativsätzen \is{Exklamativsatz} beobachtet man Unterschiede je nach Exklamativsatztyp. Aus dem Satzexklamativsatz \is{Satzexklamativsatz} scheint \textit{doch} (den Beispielen aus der Literatur folgend) (zumindest in Isolation) ausgeschlossen (vgl.\ \ref{862}).
...
\lsptoprule 	
     ...
     (q) & & (q)\tabularnewline\midrule      
   	\multicolumn{3}{l}{cg s\textsubscript{1} = $\lbrace$p > q$\rbrace$} \tabularnewline   
   \lspbottomrule
\end{tabular}
\end{exe}
...
\lsptoprule 	
   	$\textrm{DC}_{\textrm{A}}$ & {Tisch} & $\textrm{DC}_{\textrm{B}}$ \tabularnewline\cmidrule(lr){1-1}\cmidrule(lr){2-2}\cmidrule(lr){3-3}
     & p $\vee$ $\neg$p & $\neg$p\tabularnewline\midrule      
   	\multicolumn{3}{l}{cg s\textsubscript{1} = $\lbrace$p > $\neg$q, $\neg\textrm{q}\rbrace$} \tabularnewline   
   \lspbottomrule
\end{tabular}
\end{exe}
...
\lsptoprule 	
 	...
 	q & q $\vee$ $\neg$q & \tabularnewline
    & p $\vee$ $\neg$p & \tabularnewline\midrule      
   	\multicolumn{3}{l}{cg s\textsubscript{1} = $\lbrace$p > q, t > k, r > (p $\vee$ $\neg$p$)\rbrace$} \tabularnewline   
   \lspbottomrule
\end{tabular}
\end{exe}
...
\lsptoprule 	
   	$\textrm{DC}_{\textrm{A}}$ & Tisch & $\textrm{DC}_{\textrm{B}}$ \tabularnewline\cmidrule(lr){1-1}\cmidrule(lr){2-2}\cmidrule(lr){3-3}
    p & p $\vee$ $\neg$p & \tabularnewline\midrule      
   	\multicolumn{3}{l}{cg s\textsubscript{2} = $\lbrace$p > q, r, t > k, r > (p $\vee$ $\neg$p$)\rbrace$} \tabularnewline   
   \lspbottomrule
\end{tabular}
\end{exe}
...
\lsptoprule 	
   	$\textrm{DC}_{\textrm{A}}$ & Tisch & $\textrm{DC}_{\textrm{B}}$ \tabularnewline\cmidrule(lr){1-1}\cmidrule(lr){2-2}\cmidrule(lr){3-3}
 	(q) & & (q) \tabularnewline\midrule      
   	\multicolumn{3}{l}{cg s\textsubscript{1} = $\lbrace$((p $\vee$ $\neg$p) $\in$ T) \textgreater\ q$\rbrace$} \tabularnewline   
   \lspbottomrule
\end{tabular}
\end{exe}
...
\begin{exe}
	...
	\hfill\hbox{\scshape(decow2014)}\newline\relax{\scriptsize(\url{http://www.crystals-dsa-foren.de/archive/index.php/thread-2885.html})}
\end{exe}
...
\begin{table}
    ...
    \lspbottomrule	 
    \end{tabular}   
\end{table}\footnotetext{Jede Suche ist ggf. auch durch ihre Suchanfrage beschränkt. In diesem Fall sind \textit{Wo}-VL-Sätze ausgeschlossen, die von einem Fragezeichen beendet werden (Anfrage: {\ttfamily Wo /s0 MORPH(V IND -INF -PCP) /w0 \textless se> \%+w0 \textbackslash?}). Die DECOW-Daten zeigen, dass es solche Sätze gibt. Ich habe sie allein aus praktischen Gründen ausgeschlossen, um keine w-Interrogativsätze unter den Ergebnissen zu haben.}
\tabref{tab:948} bis \ref{tab:950} zeigen die Ergebnisse der parallelen Betrachtung von funktional ähnlichen kausalen Nebensätzen.
...
\begin{table}
	...
	\lspbottomrule
       \end{tabular}   
\end{table}\pagebreak\noindent
Die Webdaten geben hier folglich kein anderes Bild ab als die Daten aus DeReKo. Es ist davon auszugehen, dass \textit{doch} sowieso häufiger vorkommt als die Kombinationen aus \tabref{tab:951}. Auf deren Unterrepräsentiertheit sollte man folglich nicht schließen. Auch vermute ich, dass \textit{ja} häufiger verwendet wird als \textit{doch}, was den Kontrast noch verstärkt. Genauso ist davon auszugehen, dass MP-lose Assertionen normalerweise überwiegen (was auch \tabref{tab:948} bis \ref{tab:950} nahelegen). Für \textit{Wo}-VL-Sätze bestätigt sich in der Verwendung folglich, dass \textit{doch} zwar nicht obligatorisch ist, aber sehr typisch, z.T. in Kombination, auftritt.
...
\begin{table}
    ...
    \end{tabular}   
\end{table}
Gleiches gilt für eine exhaustive Suche nach V1-Sätzen in einem Teilkorpus von DECOW2014 (vgl.\ \tabref{tab:952}).
...
\end{exe}										  
Ein anderer Punkt, der für mich gegen die vorgenommenen Eigenschaftszuschreibungen (Hintergrund, Bekanntheit, Faktum, Unkontroversität) spricht, ist auch, dass man im \textit{Wo}-Satz das Vorkommen von \textit{ja} erwarten würde, als beliebte Partikel an der Stelle, weil diese unmissverständlich Bekanntheit, Faktenorientierung etc. kodiert. Die Verteilungen in \tabref{tab:947} und \ref{tab:951} zeigen aber, dass \textit{ja} hier nicht auffällig beliebt ist.
...
In einer Korpusstudie (Protokolle von Parlamentsreden von Helmut Kohl) be\-stimmt sie die Häufigkeit des Auftretens einer MP in Verbindung mit einer bestimmten Diskursrelation relativ zu einem Erwartungswert für das unabhängige Auftreten der untersuchten \is{Diskursrelation} Diskursrelationen. 
...
\end{exe}
Ein anderer Einwand betrifft die vollkommentarische Natur \is{vollkommentarisch} von V1-De\-kla\-ra\-tiv\-sät\-zen. 
	...
	\ex\label{970} V1-Deklarativsatztypen im Deutschen\\[-1em]
		\begin{xlist}	
			...
			\ex\label{970b} aufzählend-reihende V1-Deklarative \is{auf\-zähl\-end-reihender V1-De\-kla\-ra\-tiv\-satz}
				...
				\end{xlist}
			\ex\label{970c} inhaltlich-begründende V1-Deklarative \is{inhaltlich-begründender V1-De\-kla\-ra\-tiv\-satz}\\\relax
...
\begin{exe}
	...
	So gesehen, könnte den beiden Lindenau-Grafen eigentlich der Titel „Tierarzt ehrenhalber“ zugeordnet und ihr Wiegenort Machern als eine Art 		„Wallfahrtsort“ für Tierärzte und Pferdezüchter gepriesen werden. \textbf{\textit{Gibt} es \ul{doch} \textsc{hier} viele Andenken an 		beide:} das Schloss mit der Ritterstube, die drei Grafen-Wappen und die Lindenau-Ausstellung wie auch den Landschaftsgarten mit seinen Parkbauten und 		Plastiken. 		
	\hfill\hbox{\scshape(decow14ax)}
	\newline
	{\scriptsize(\url{http://www.uni-leipzig.de/\~{}mielke/MachernH/vetarzte.htm})}
\end{exe}

\begin{exe}
	\ex\label{1001} 

	Robbins-Fans werden sich über dieses neue Buch freuen. Nicht so unsere Wende-Politiker. \textbf{\textit{Handelt} es sich \ul{doch} hierbei um ein einziges 				enthusiastisches Plädoyer für den Terrorismus:} „Die Terroristen sind die Büchsenöffner im Supermarkt des Lebens“.		
	\hfill\hbox{\scshape(decow14ax)}
	\newline
	{\scriptsize(\url{http://blogs.taz.de/hausmeisterblog/2007/01/page/5/})}
\end{exe}
Ich frage mich, ob diese Einheiten nicht eher die Funktion haben, einzuordnen, in welche Szene der Referent/das Thema eingeführt wird, als das Prädikat mit seinen Argumenten auf einen Satzgegenstand zu beziehen. Dies gilt deutlich für Fälle der Art in (\ref{1000}) und (\ref{1001}), aber auch für (\ref{996}) und (\ref{997}) und z.T. auch für Beispiele der Art in  (\ref{991}) bis (\ref{995}). Mit Ausnahme von fünf Fällen (wobei in zweien grammatische Gründe vorliegen) könnten die \textquote{Topikkonstituenten} auch weggelassen werden, ohne dass die Sätze anders interpretiert würden (vgl.\ z.B.\ \ref{995} und \ref{1002} vs. \ref{1004}).

\begin{exe}
	\ex\label{1002} 

	Sogar Diesel durften jetzt sportlich sein, wie der Ibiza Cupra zum Modelljahr 2004 	zeigte. \textbf{\textit{Gab} es \textsc{ihn} \ul{doch} 			erstmals als 160 PS starken 1,9-Liter-TDI.} 		
	\hfill\hbox{\scshape(decow14ax)}
	\begin{sloppypar}
	{\scriptsize(\url{http://ww2.autoscout24.at/test/renault-zoe/so-wird-das-nichts-frau-merkel/4319/418106/mrb-mz\_home?article=420574\%26intcidm=mrb-mz\_home})}\end{sloppypar}
\end{exe}	
...
In DeReKo treten die \textit{Wo}-Sätze vornehmlich in Texten auf, die man dem Feuilleton zuordnen würde, wie z.B.\ Kolumnen. In diesem Sinne liegen recht \textquote{lockere} Kontexte vor. Typisch sind Fälle wie in (\ref{1048}) und (\ref{1054}).
...
\multicolumn{3}{l}{cg s\textsubscript{3} = $\lbrace$s\textsubscript{2} $\cup \ \lbrace$!p $\in$ $\textrm{TDL}_{\textrm{B}}\rbrace\rbrace$} \tabularnewline
\lspbottomrule
\end{tabular}
\end{exe}}
...
\cmidrule(lr){1-1}\cmidrule(lr){3-3}
{} & {} & {}  \tabularnewline\midrule
\multicolumn{3}{l}{cg s\textsubscript{1} = $\lbrace\neg$q, p > q$\rbrace$} \tabularnewline
\lspbottomrule
\end{tabular}
\end{exe}		 
...
\begin{exe}
	...
	\newline{\scriptsize(\url{http://www.abendblatt.de/archiv/1965/article200886539/Sind-wir-wirklich-unpuenktlich.html})}		
\end{exe}
...
\cmidrule(lr){1-1}\cmidrule(lr){3-3}
{} & {} & !p  \tabularnewline\midrule
\multicolumn{3}{l}{cg s\textsubscript{2} = $\lbrace$s\textsubscript{1} $\cup$ $\lbrace$!p $\in$ $\textrm{TDL}_{\textrm{B}}\rbrace$} \tabularnewline
\lspbottomrule
\end{tabular}
\end{exe}										 
...
\begin{exe}
	...
	B: Ihr wart \textbf{auch} artig dieses Jahr. (p)\\\relax
	...
	\ex\label{1136} 
		\begin{xlist}
			...
			\ex\label{1136b} A: Wir wollen, dass der Nikolaus nett zu uns ist. (q)\\
							 B: Dann seid das Jahr über \textbf{auch} artig! (!p)\\\relax
		[q > !p ‚Wenn ihr wollt, dass der Nikolaus nett ist, müsst ihr das Jahr über artig sein.‘]					 
		\end{xlist}
\end{exe}	
...
\begin{exe}
	...
	B: Warum wart ihr \textbf{auch} nicht artig dieses Jahr?\\\relax
	[p > q ‚Wenn ihr artig wart, ist der Nikolaus nett zu euch.‘\\
...
\begin{exe}
	...
	(mit Antwortpräferenz zu p, weil sie wollen, dass der Nikolaus nett zu ihnen ist)\\\relax
	[p > q ‚Wenn ihr artig wart, ist der Nikolaus nett zu euch.‘]
\end{exe}
...
Als nächstes wären auf einer \textquote{Skala} von MP-Adverb-Qualität unpersönliche Strukturen mit Modalverben einzuordnen (vgl.\ \ref{1146}).
...
\begin{exe}
	...
	\hbox{}\hfill\hbox{(\textsc{brz13/mai.05814} Braunschweiger Zeitung, 16.05.2013)}}\\\relax
...
\begin{exe}
	...
	\hbox{}\hfill\hbox{(\textsc{a97/jun.09605} St. Galler Tagblatt, 16.06.1997)}}\\\relax
...
\begin{exe}
	...
	\newline\hbox{}\hfill\hbox{(\textsc{a97/aug.19304} St. Galler Tagblatt, 20.08.1997)}}\\\relax
...
Ähnlich kann man in (\ref{1159}) als Relation, auf die \textit{auch} Bezug nimmt, ansetzen: ‚Wenn die Verhältnisse auf bestimmte Art beschaffen sind, dann teilt diese mit.‘ Bis zum Sprechzeitpunkt sagen die Angesprochenen dies scheinbar nicht, woraus sich ergibt, dass fraglich ist, ob sie es sagen werden. Auf diese offene Frage bezieht sich \textit{doch}.
...
\cmidrule(lr){1-1}\cmidrule(lr){3-3}
{} & {} & {}  \tabularnewline\midrule
\multicolumn{3}{l}{cg s\textsubscript{1} = $\lbrace$q > !((p $\vee$ $\neg$p) $\in$ T)$\rbrace$} \tabularnewline
\lspbottomrule
\end{tabular}
\end{exe}}
...
\subsection{Der Ausschluss der \textquote{Dubletten}}
Ich gehe deshalb auch im Falle dieser Kombination davon aus, dass aus den seltenen Treffern in DeReKo und DGD2 nicht auf die Non-Existenz und Ungrammatikalität dieser Reihung zu schließen ist. Bei der Beschäftigung mit MPn hat man stets mit dem Umstand umzugehen, dass es sich bei einer betrachteten \textquote{MP} um eine ihrer \textquote{Dubletten}  handeln könnte. Vor diesem Hintergrund ist die Untersuchung des kombinierten Auftretens der Partikeln \textit{doch} und \textit{auch} deshalb besonders problematisch, da prinzipiell beide nicht in ihrer MP-Verwendung vorliegen können.
...
\begin{exe}
	...
	\newline(\textsc{h87/bm5.11485} Mannheimer Morgen, 25.04.1987, S. 05; Die fetten Jahre)\\\relax
...
\begin{exe}
	...
	\newline(\textsc{k97/jun.42849} Kleine Zeitung, 08.06.1997, Ressort: Lokal)\\\relax	
...
\begin{exe}
	...
	\newline(\textsc{z04/405.04570} Die Zeit (Online-Ausgabe), 27.05.2004)\\\relax	
...
\begin{exe}
	...
	\newline
	(\textsc{wdd11/l43.00760}: Diskussion:Lectorium Rosicrucianum\slash Archiv\slash 2010\slash\linebreak 1. Teilarchiv) [MP \textit{auch}, Adverb \textit{doch}]
\end{exe}
...
\begin{exe}
	...
	\newline{\scriptsize(\url{http://www.websitepark.de/forum/archive/index.php/t-4754.html})}\hfill\hbox{\scshape(decow2014)}	
...
Eine ähnliche Erklärung über die Verarbeitung bekannter Teilstrukturen lässt sich auch für das Auftreten von \textit{auch doch} in den kausalen Nebensätzen andenken: Stellte sich heraus, dass \textit{auch} in kausalen Nebensätzen frequent wäre (meine Verteilungsangaben zu \textit{da}-, \textit{denn}- und \textit{zumal}-Sätzen legen dies in Abschnitt~\ref{sec:korp} für \textit{zumal}-Sätze nahe), könnte auch das Vorkommen von \textit{auch} als Muster gelten, dessen Bekanntheit/Erwartetheit ausreicht, um ein hinzutretendes \textit{doch}, das an dieser Stelle eigentlich nicht gewünscht ist, zu \textquote{tolerieren}.\\
