...
\ChapterDOI{10.5281/zenodo.1182589}
...
\shorttitlerunninghead{Semantic and syntactic patterns of multiword names}
...
\lehead{S. Koeva, C. Krstev, D. Vitas, T. Kyriacopoulou, C. Martineau \& T. Dimitrova}
\maketitle
...
classes:  \is{name!personal name} personal names, \is{name!location name} location names, and \is{name!organization name} organization names, called
also \is{name} named entities (NEs). They can be single-word nouns or particular
...
mentioned explicitly in the text. Furthermore, multiword names \is{name!multiword name} may
...
semantic patterns \is{pattern} we propose show semantic compatibility valid for a
...
The syntactic patterns \is{pattern} are language-specific and differ for personal,
...
\is{clitic} clitics (in \ili{Bulgarian} and \ili{Greek}), etc.
...
\item[\ili{Bulgarian}:] (((DefAdj (PossCL)?) | \textbf{DefPossPron}) Adj* \trigger{Trigger}
...
\item[\ili{French}:] (((DefArt|PossPron) Adj* \trigger{Trigger} (PP 
...
\item[\ili{Greek}:] ((DefArt Adj* (PossPron)? \trigger{Trigger} (GenP \textit{= DomSpec})?
...
\item[\ili{Serbian}:] ((Adv | PossPron)? DefAdj* \trigger{Trigger} (PP \textit{za}| GenP =
...
\begin{description}
\item[\ili{English}:] ((DefArt Adj* \trigger{Trigger} (PP \textit{of = Aff})* (PP
...
\item[\ili{Bulgarian}:] (((DefAdj (PossCL)? | DefPossPron) Adj* \trigger{Trigger} (PP
...
\item[\ili{French}:] ((DefArt Adj* ~~~\trigger{Trigger} (PP \textit{de = Aff})* ~~(PP
\textit{à}|\textit{en = Loc})?) | (DefArt \trigger{Trigger}* Adj* (PP \textit{de =
Aff})* (PP \textit{à}|\textit{en = Loc})?) | \trigger{IndefTrigger}) \textbf{PerN}

 

\item[\ili{Greek}:] ((DefArt Adj* \trigger{Trigger} (GenP \textit{= Aff})* (GenP\textit{ =
Loc})?) | (DefArt \trigger{Trigger} (GenP = \textit{Aff} | \textit{Loc})* (PP
σε \textit{= Loc})?) | \trigger{DefTrigger}) \textbf{PerN}

 

\item[\ili{Serbian}:] (((PossPron | PossAdj) ~~\trigger{Trigger}) | (DefAdj* ~~\trigger{Trigger})
...
\item[\ili{English}:] (((DefArt | GenDet | PossPron) ~~Adj*~~ \trigger{Trigger} (PP \textit{from
...
\item[\ili{French}:] (DefArt | PossPron) Adj* \trigger{Trigger} ((PP \textit{de = } \textbf{PerN}) |
(PP \textit{de = Loc})?)  \textbf{PerN}

 

\item[\ili{Greek}:] (DefArt Adj* \trigger{Trigger} ((GenP =  \textbf{PerN}) | (GenP =
\textit{Poss}))? (PP από DefArt \textit{= Loc})?) DefArt  \textbf{PerN}

 

\item[\ili{Serbian}:] (\textbf{PerN}* \trigger{Trigger}* (DefAdj* | (GenP =  \textbf{PerN}))? (PP \textit{iz
...
\item[\ili{English}:] (DefArt Adj* ~~\trigger{Trigger} (PP \textit{of}|\textit{from = Loc})?
...
\item[\ili{French}:] (DefArt Adj* \trigger{Тrigger} (PP \textit{de = Loc})? \textbf{PerN})
| (\trigger{Trigger} Adj* \textbf{PerN }(PP \textit{de = Loc})?)

 

\item[\ili{Greek}:] (DefArt Adj* \trigger{Trigger} (GenP \textit{= Loc})? \textbf{PerN}) |
(DefArt \trigger{Trigger} \textbf{PerN }(GenP \textit{= Loc})? )

 

\item[\ili{Serbian}:] (PossPron? DefAdj* \trigger{Trigger} ((PP \textit{iz}|\textit{u}|\textit{sa =
...
\item[\ili{English}:] \ili{Bulgarian}: \ili{French}: \trigger{IndefTrigger} \textbf{PerN}

 

\item[\ili{Greek}:] DefArt \trigger{Trigger} \textbf{PerN}

 

\item[\ili{Serbian}:] (\trigger{Trigger} \textbf{PerN) }\trigger{|} \textbf{(PerN
}\trigger{Trigger})
...
\textbf{Aix-en-Provence}’; iii) \is{name!location names} Location names can be MWEs. Examples:
...
\item[\ili{English}:] (((DefArt | GenDet | PossPron) Adj* \trigger{Trigger} (PP
\textit{of = Spec})? (PP \textit{in = Loc})?) | (DefArt
\trigger{Trigger}\textbf{ }(PP \textit{of = Spec})?) |
\trigger{IndefTrigger}) \textbf{LocN}
 

\item[\ili{Bulgarian}:] (((DefAdj (PossCL)? | DefPossPron) Adj* \trigger{Trigger} (PP \textit{na
...
\item[\ili{French}:] (((DefArt | PossPron) Adj* \trigger{Trigger} (PP \textit{de = Spec})?
...
\item[\ili{Greek}:] (((DefArt | PossPron) Adj* \trigger{Trigger} (GenP = \textit{Spec})?
(PP σε \textit{= Loc})?) | \linebreak (DefArt \trigger{Trigger} (GenP)?))
\textbf{LocN}

 

\item[\ili{Serbian}:] ((PossPron | PossAdj)? DefAdj* \trigger{Trigger} (PP
...
\item[\ili{English}:] ((DefArt | GenDet | PossPron) Adj* \textbf{MWLocN }(PP \textit{in =
Loc})?) | (DefArt \textbf{MWLocN }(PP \textit{in = Loc})?) |
\textbf{MWLocN}

 

\item[\ili{Bulgarian}:] (((DefAdj (PossCL)?) | DefPossPron) Adj* \textbf{MWLocN} (PP \textit{v}|\textit{na}|\textit{pri = Loc})?) | (\textbf{DefMWLocN} (PP \textit{v}|\textit{na}|\textit{pri =
Loc})?) | \textbf{MWLocN}

 

\item[\ili{French}:] ((DefArt | PossPron) Adj* \textbf{MWLocN} (PP \textit{de = Loc})?) |
(\textbf{MWLocN} (PP \textit{de = Loc})?) | \textbf{MWLocN}

 

\item[\ili{Greek}:] ((DefArt | PossPron) Adj* \textbf{MWLocN} (GenP\textit{ = Loc})?) |
(\textbf{DefMWLocN} (GenP \textit{= Loc})?) | \textbf{MWLocN}

 

\item[\ili{Serbian}:] ((PossPron | PossAdj)? DefAdj* (PP \textit{u}|\textit{na}|\textit{pri}|\textit{pod}|\textit{u blizini }|
...
\item[\ili{English}:] (((DefArt | GenDet | PossPron) Adj* \trigger{Trigger}\textbf{ }(PP
...
\item[\ili{French}:] (((DefArt | PossPron) Adj* \trigger{Trigger} (PP \textit{de =
...
\item[\ili{Greek}:] ((DefArt Adj* \trigger{Trigge}r (PossPron)? ((PP για = \textit{
...
\item[\ili{Serbian}:] (PossPron? DefAdj* \trigger{Trigger} (PP \textit{za} | \textit{GenP =
...
\item[\ili{English}:] (DefArt | PossPron) Adj* \textbf{MWOrgN }(PP \textit{at}|\textit{in = Loc})?)
| (\textbf{DefMWOrgN }(PP \textit{at}|\textit{in = Loc})?) | \textbf{MWOrgN}

 

\item[\ili{Bulgarian}:] (((DefAdj (PossCL)?) | DefPossPron) Adj* \textbf{MWOrgN} (PP
\textit{v}|\textit{na}|\textit{pri = Loc})?) | (\textbf{DefMWOrgN} (PP \textit{v}|\textit{na}|\textit{pri =
Loc})?) | \textbf{MWOrgN}

 

\item[\ili{French}:] (DefArt | PossPron) Adj* \textbf{MWOrgN} (PP \textit{à} \textit{de = Loc})?

 

\item[\ili{Greek}:] ((DefArt | PossPron) Adj* \textbf{MWOrgN} (PP σε \textit{= Loc})?) |
(DefArt \textbf{MWOrgN} (GenP = \textit{Loc})?) | \textbf{MWOrgN}

 

\item[\ili{Serbian}:] (PossPron? DefAdj* \textbf{MWOrgN} (GenP = \textit{Poss})? (PP \textit{iz}|\textit{u}| \textit{GenP} = \textit{Loc})?) | \textbf{MWOrgN}
\end{description}
...
The syntactic patterns of \ili{English} proper name triggers involve
...
In \ili{Bulgarian}, the syntactic patterns for proper {name triggers exhibit
...
possessive pronoun \isi{clitic} in post-nominal position (\textit{moyata
...
\textbf{Paris}’. In that case, the use of the possessive pronoun \isi{clitic}
...
With personal names in \ili{English}, the definite article is obligatory when
...
\subsection{Distribution of clitics in {Bulgarian} and {Greek}} %7.3. /
...
personal names, as the first name and surname(s) can change places: \ili{French}:
...
of-\ili{French}.Adj \textbf{François Mitterrand}’).
...
\midrule
% \ili{English} &  \ili{Bulgarian} &  \ili{French} &  \ili{Greek} &  \ili{Serbian}\\
...
multiword names in \ili{Bulgarian}, \ili{English}, \ili{French}, \ili{Greek}, and \ili{Serbian}, may
