...
Das ist vergleichbar mit der zugrundeliegenden Form der Phonologie (Abschnitt~\ref{sec:segmentemerkmaleverteilungen}), die ebenfalls genau die Information enthält, die benötigt wird, um alle phonetischen Realisierungen eines Segments in allen möglichen Kontexten abzuleiten.

\newcommand{\DefWort}{Das (\textit{lexikalische}) \textit{Wort} ist eine Repräsentation von paradigmatisch zusammengehörenden Wortformen (also umgangssprachlich die Zusammenfassung bzw.\ Abstraktion aller Formen eines Wortes).
Beim lexikalischen Wort sind die Werte nur für diejenigen Merkmale spezifiziert, die in allen Wortformen des Paradigmas dieselben Werte haben.
...
\begin{exe}
  \ex{\label{ex:wa4282} Tisch (lexikalisches Wort) =\\
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex[]{\label{ex:wa-6622c} Alexandra und Dzsenifer spielen eine gute Saison.}
    \ex[*]{\label{ex:wa-6622d} Alexandra obwohl Dzsenifer spielen eine gute Saison.}
  \end{xlist}
  \ex\label{ex:wa6623}
  \begin{xlist}
...
\begin{exe}
  \ex\label{ex:wk9123}
  \begin{xlist}
    \ex{\label{ex:wk9123a} Barbara läuft.}
    \ex{\label{ex:wk9123b} Barbara lief.}
  \end{xlist}
\end{exe}
...
\WFiltTree[0cm]{Präposition}{wfilt:prep}{nicht flektierbares Wort}{Hat das Wort eine einstellige Valenz und Kasusrektion?}{Präposition}{andere}
...
\begin{exe}
  \ex\label{ex:wa9911}\begin{xlist}
    \ex[]{\label{ex:wa9911a} Ich glaube, [dass dieser Nebensatz ein Verb enthält].}
    \ex[]{\label{ex:wa9911b}{} [Während die Spielzeit läuft], zählt jedes Tor.}
    \ex[]{\label{ex:wa9911c} Es fällt ihnen schwer [zu laufen].}
    \ex[*]{\label{ex:wa9911d} [Obwohl kein Tor fiel].}
  \end{xlist}
\end{exe}

In (\ref{ex:wa9911a}) ist die Definition des Nebensatzes erfüllt, weil \textit{enthält} ein \textsc{Tempus}-Merkmal hat und damit finit ist.
...
\begin{exe}
  \ex\label{ex:wa2829}
  \begin{xlist}
    \ex[]{\label{ex:wa2829a}Gestern hat der FCR Duisburg gewonnen.}
    \ex[]{\label{ex:wa2829b}Erfreulicherweise hat der FCR Duisburg gestern gewonnen.}
    \ex[]{\label{ex:wa2829c}Oben finden wir andere Beispiele.}
    \ex[*]{\label{ex:wa2829d}Doch ist das aber nicht das Ende der Saison.}
    \ex[*]{\label{ex:wa2829e}Und ist die Saison zuende.}
  \end{xlist}
...
\WFiltTree[0cm]{Partikel}{wfilt:advpart}{Partikel\slash Adverb}{Ist das Wort ein Vorfeldbesetzer?}{Adverb\slash Adkopula}{Partikel}
...
Die Beispielsätze in (\ref{ex:wa2220}) zeigen Adkopulas, die jeweils mit einem sogenannten \textit{Kopulaverb} (KoV) wie \textit{sein}, \textit{bleiben} oder \textit{werden} auftreten.\index{Kopula}
...
\begin{exe}
  ...
  \begin{xlist}
  	...
  	\ex[ ]{\label{ex:wa0815b} Die starke Gewichtheberin ist Weltmeisterin.}
  \end{xlist}
  \ex\label{ex:wa0816}
  \begin{xlist}
  	\ex[ ]{\label{ex:wa0816a} Der Staat ist pleite.}
  	\ex[*]{\label{ex:wa0816b} Der pleite Staat bricht zusammen.}
  \end{xlist}
\end{exe}
...
\begin{figure}[!htbp]
  ...
  \resizebox{\textwidth}{!}{
    ...
    && \K{\textbf{Adjektiv}} && \K{\textbf{Artikel\slash}}\Below{\textbf{Pronomen}} &&& \K{Adverb\slash Adkopula}\B{d} &&&& \K{Partikel}\B{d} \\
    &&&&&&& \K{(F8) protot.\ mit KoV?}\Bjn &&&& \K{(F9) satzersetzend?}\Bjn \\
    &&&&&& \K{\textbf{Adkopula}} && \K{\textbf{Adverb}} && \K{\textbf{Satzäquivalent}} && \K{andere}\B{d} \\
...
\Uebung[\tristar] \label{u57} Überlegen Sie, warum man Wörter wie \textit{darin}, \textit{dabei}, \textit{darunter}, \textit{damit} usw.\ als \textit{Pronominaladverben} bezeichnet.\index{Pronominaladverb}
